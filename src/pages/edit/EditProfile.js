...
const EditProfile = () => {
  ...
  const profile = Session.getProfile(Session.getSessionData().username);

  const [file, setFile] = useState(profile.img);
  ...
  const handleSumbit = (e) => {
    ...
    newProfile["img"] = file;
    newProfile["showcase"] = showcase
    Session.updateProfile(newProfile);
    Session.redirectTo(e, `/profile?name=${profile.username}`)
  }
  ...
  return (
    <div className='grid grid-cols-4 gap-4 mx-24'>
      <div className='pr-4 border-r-2 mt-6'>
        ...
        <h1 className='text-xl font-semibold text-center'>{profile.username}</h1>
      </div>
      <div className='w-full h-min col-span-3 mb-4'>
        ...
        <div className='h-min relative'>
          ...
          />
          <p className='absolute bottom-4 right-4'>{description.length} / 200</p>
        </div>
        <button
          type="button"
